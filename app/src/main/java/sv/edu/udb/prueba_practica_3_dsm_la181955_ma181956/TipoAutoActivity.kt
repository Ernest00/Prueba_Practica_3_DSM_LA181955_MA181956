...
class TipoAutoActivity : AppCompatActivity(),  View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_tipo_auto)//activity
        ...
        if (db != null) {
            if (view === btnAgregar) {
                if (vericarFormulario("insertar")) {
                    managerTipo!!.addNewItem(
                        nombre
                    )
                    txtNombre!!.setText("")
                    txtId!!.setText("")
                    mostrarMensajesArriba(view, "Tipo agregado")

                }
            } else if (view === btnActualizar) {
                if (vericarFormulario("actualizar")) {
                    managerTipo!!.updateItem(
                        idSel.toInt(),
                        nombre
                    )
                    txtId!!.setText("")
                    txtNombre!!.setText("")
                    mostrarMensajesArriba(view, "Tipo actualizado")

                }
            } else if (view === btnEliminar) {
                if (vericarFormulario("eliminar")) {
// manager.eliminar(1);
                    managerTipo!!.deleteItem(idSel.toInt())
                    txtId!!.setText("")
                txtNombre!!.setText("")
                    mostrarMensajesArriba(view, "Tipo eliminado")

                }
            } else if (view === btnBuscar) {

                if(vericarFormulario("buscar")){
                    cursor= managerTipo!!.searchItem(idSel.toInt())
    ...
    private fun vericarFormulario(opc: String): Boolean {
        ...
        if (opc === "insertar" || opc == "actualizar") {
            if (nombre.isEmpty()) {
                txtNombre!!.error = "Ingrese la descripción del tipo de auto"
            ...
            if (opc == "actualizar") {
                if (idproducto.isEmpty()) {
                    ...
                    notificacion = "No se ha seleccionado un tipo de auto"
                }
